...
/*!
    \brief     init usar2 串口2初始化  BL0942使用
    \param[in]  none
    \param[out] none
    \retval     none
*/
...
/*!
    \brief     init usar3 串口3初始化  RS485使用
    \param[in]  none
    \param[out] none
    \retval     none
*/
...
/*!
    \brief     init usar4 串?4初始化  RS485使用
    \param[in]  none
    \param[out] none
    \retval     none
*/
...
/*!
    \brief     Usart5_RxOverTime
    \param[in]  none
    \param[out] none
    \retval     none
*/
void Usart5_RxOverTime(void)
{
    Uart5.RxFlag = 1;
...
void UART5_IRQHandler(void)
{
    ...
    if (USART_GetIntStatus(UART5, USART_INT_RXDNE) != RESET)
    {
        if (Uart5.RxFlag == 0)
        {
            rec = USART_ReceiveData(UART5);
            Uart5.RxBuf[Uart5.RxLen] = rec;
            Uart5.RxLen++;
            Vtimer_KillTimer(VTIM7);
            Vtimer_SetTimer(VTIM7, 50, Usart5_RxOverTime);
        }
        else
            rec = USART_ReceiveData(UART5);
    }
...
/*!
    \brief    Uart5_UartStaus
    \param[in]  none
    \param[out] none
    \retval     none
*/
uint8_t Uart5_UartStaus(void)
{
    if (Uart5.RxFlag == 1)
        return 1;
    else
...
/*!
    \brief    Uart5ClearRxBuf
    \param[in]  none
    \param[out] none
    \retval     none
*/
void Uart5ClearRxBuf(void)
{
    uint8_t i;
    for (i = 0; i < Uart5.RxLen; i++)
        Uart5.RxBuf[i] = 0;
    Uart5.RxLen = 0;
    Uart5.RxFlag = 0;
}
...
/*!
    \brief    Uart5CopyRxData
    \param[in]   uint8_t *
    \param[out]  uint8_t *
    \retval     none
*/
void Uart5CopyRxData(uint8_t *data, uint8_t *len)
{
    uint8_t i;
    for (i = 0; i < Uart5.RxLen; i++)
        data[i] = Uart5.RxBuf[i];
    *len = Uart5.RxLen;
    Uart5ClearRxBuf();
}
...
/*!
    ...
    \retval     none
*/
void Uart5Send(u8 *data, u16 len)
{
    u16 i;
    for (i = 0; i < len; i++)
    {
