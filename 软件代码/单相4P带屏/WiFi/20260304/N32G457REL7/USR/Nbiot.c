...
u8 SetPDP_Context(void)
{
    ...
    case 0:
        querynum++;
        NB_ClearRxBuf();
        Uart4Send("AT+QICSGP=1,1,\"CMNET\",\"\",\"\",1\r\n", 31); // 关闭回显
        NB_Start_TimeOutTimer(100);
        state = 1;
        return 0;
    case 1:
...
u8 MQTT_Creat(void)
{
    ...
    case 2:
        NB_ClearRxBuf();
        Uart4Send((u8 *)"AT+MQTTCFG=\"keepalive\",0,120\r\n", 30);
        NB_Start_TimeOutTimer(300);
        state = 3;
        return 0;
    case 3:
    ...
    case 4:
        ...
        Uart4Send((u8 *)stringlen, strlen(stringlen));
        Uart4Send(",\"", 2);
        Uart4Send(g_Equipment_number, 16);
        Uart4Send("\",", 2);
        Uart4Send("\"user\",\"0.123456\"\r\n", 19);
    ...
    case 6:
        NB_ClearRxBuf();
        Uart4Send("AT+MQTTSUB=0,\"modbus_control/", 29);
    ...
    case 8:
        NB_ClearRxBuf();
        Uart4Send("AT+MQTTSUB=0,\"modbus_time/", 26);
        sprintf(stringlen, "%s", g_Equipment_number);
        Uart4Send((u8 *)stringlen, strlen(stringlen));
        Uart4Send("\",0\r\n", 5);
        NB_Start_TimeOutTimer(5000);
  ...
  //  char *p4 = NULL;
    ...
    case 3:
        if (NB_Check_TimeOutTimer()) // 超时时间到
        {
            ...
            {
//                p4 = strstr((const char *)g_NBbuf, (const char *)"+CME ERROR: 601");
//                if (p4 != NULL)
//                {
//                    m_readateerrnum = 0;
//                } 
