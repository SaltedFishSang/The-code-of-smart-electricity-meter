...
#define SwitchTimeResetValue 0 // 定时开关默认值
...
void Eprom_write_resetData(void)
{
    ...
    EEP_Write8(SwitchTime, SwitchTimeResetValue);
}
...
void Eprom_read_Data(void)
{
    ...
    Switch_Status.SwitchTimevalue = EEP_Read8(SwitchTime);
#if EPROM_DBG
    ...
    printf("Switch_Status.SwitchTimevalue == %d\r\n", Switch_Status.SwitchTimevalue);
#endif
    Relay_Control(Switch_Status.SwitchOnvalue);
    DisplayClockState(Switch_Status.SwitchTimevalue); // 显示定时
    DisplayLockState(Switch_Status.SwitchLockvalue);  // 显示本地开关使能禁止
    DisplayOutputState(Switch_Status.SwitchOnvalue);  // 显示输出打开还是关闭
}

/*!
    \brief      Eprom_write_ResetTimerPoint(void)
    \param[in]  none
    \param[out] none
    \retval     none
*/
void Eprom_write_ResetTimerPoint(void)
{
    u8 i = 0;
    /*默认两个点*/
    /*第一个点：0点0分 周一到周日 开*/
    strcpy(TimerPoint[0].weekday, "");
    strcat(TimerPoint[0].weekday, "1234567");
    TimerPoint[0].hour = 0;
    TimerPoint[0].min = 0;
    TimerPoint[0].onoff = 1;
    /*第二个点：23点59分 周一到周日 关*/
    strcpy(TimerPoint[1].weekday, "");
    strcat(TimerPoint[1].weekday, "1234567");
    TimerPoint[1].hour = 23;
    TimerPoint[1].min = 59;
    TimerPoint[1].onoff = 0;

    for (i = 2; i < 20; i++)
    {
        strcpy(TimerPoint[i].weekday, "0");
        TimerPoint[i].hour = 0;
        TimerPoint[i].min = 0;
        TimerPoint[i].onoff = 0;
    }

    /*存储时间点   周一到周日字符串 1234567默认长度8（7+1）需要8个字节
                  时间一个字节
                  分钟一个字节
                  开关一个字节
                  一共需要11个字节
                  20个点就需要 20*11=220个存储位置
                   */
    for (i = 0; i < 20; i++)
    {
        EEP_Write8(SwitchTimePoint + 11 * i, TimerPoint[i].weekday[0]);
        EEP_Write8(SwitchTimePoint + 11 * i + 1, TimerPoint[i].weekday[1]);
        EEP_Write8(SwitchTimePoint + 11 * i + 2, TimerPoint[i].weekday[2]);
        EEP_Write8(SwitchTimePoint + 11 * i + 3, TimerPoint[i].weekday[3]);
        EEP_Write8(SwitchTimePoint + 11 * i + 4, TimerPoint[i].weekday[4]);
        EEP_Write8(SwitchTimePoint + 11 * i + 5, TimerPoint[i].weekday[5]);
        EEP_Write8(SwitchTimePoint + 11 * i + 6, TimerPoint[i].weekday[6]);
        EEP_Write8(SwitchTimePoint + 11 * i + 7, TimerPoint[i].weekday[7]);
        EEP_Write8(SwitchTimePoint + 11 * i + 8, TimerPoint[i].hour);
        EEP_Write8(SwitchTimePoint + 11 * i + 9, TimerPoint[i].min);
        EEP_Write8(SwitchTimePoint + 11 * i + 10, TimerPoint[i].onoff);
    }
}
/*!
    \brief      Eprom_write_TimerPoint(void)
    \param[in]  none
    \param[out] none
    \retval     none
*/
void Eprom_write_TimerPoint(void)
{
    u8 i = 0;
    /*存储时间点   周一到周日字符串 1234567默认长度8（7+1）需要8个字节
                  时间一个字节
...
/*!
    ...
    \retval     none
*/
void Eprom_read_TimerPoint(void)
{
    u8 i = 0;
    for (i = 0; i < 20; i++)
    {
        TimerPoint[i].weekday[0] = EEP_Read8(SwitchTimePoint + 11 * i);
        TimerPoint[i].weekday[1] = EEP_Read8(SwitchTimePoint + 11 * i + 1);
        TimerPoint[i].weekday[2] = EEP_Read8(SwitchTimePoint + 11 * i + 2);
        TimerPoint[i].weekday[3] = EEP_Read8(SwitchTimePoint + 11 * i + 3);
        TimerPoint[i].weekday[4] = EEP_Read8(SwitchTimePoint + 11 * i + 4);
        TimerPoint[i].weekday[5] = EEP_Read8(SwitchTimePoint + 11 * i + 5);
        TimerPoint[i].weekday[6] = EEP_Read8(SwitchTimePoint + 11 * i + 6);
        TimerPoint[i].weekday[7] = EEP_Read8(SwitchTimePoint + 11 * i + 7);
        TimerPoint[i].hour = EEP_Read8(SwitchTimePoint + 11 * i + 8);
        TimerPoint[i].min = EEP_Read8(SwitchTimePoint + 11 * i + 9);
        TimerPoint[i].onoff = EEP_Read8(SwitchTimePoint + 11 * i + 10);
#if _Printf_DBG
        printf("TimerPoint[%d].weekday=%s\r\n", i, TimerPoint[i].weekday);
        printf("TimerPoint[%d].hour=%d\r\n", i, TimerPoint[i].hour);
        printf("TimerPoint[%d].min=%d\r\n", i, TimerPoint[i].min);
        printf("TimerPoint[%d].onoff=%d\r\n", i, TimerPoint[i].onoff);
        printf("\r\n\r\n\r\n");
#endif
    }
}
...
void Eprom_paraInit(void)
{
    ...
    if ((table[0] == 0xa1) && (table[1] == 0x58) && (table[2] == 0x59) && (table[3] == 0xa2))
    {
        ...
        Eprom_read_Data();
        // Eprom_write_ResetTimerPoint();
        Eprom_read_TimerPoint();
    }
    else
    {
#if _Printf_DBG
