...
void HeartInfoLimit(void)
{
    ...
    if (send2 != NULL)
    {
        strcpy(send2, "");
        strcat(send2, "[");
        sprintf(send3, "{ \"k\":\"On\",\"v\":\"%s\"},", (Switch_Status.SwitchOnvalue == 1) ? "on" : "off");
        strcat(send2, send3);
        sprintf(send3, "{ \"k\":\"Lock\",\"v\":\"%s\"},", (Switch_Status.SwitchLockvalue == 1) ? "on" : "off");
        strcat(send2, send3);
        ...
        strcat(send2, send3);
        sprintf(send3, "{ \"k\":\"OverV\",\"v\":\"%d\"},", Switch_Status.OverVValue);
        strcat(send2, send3);
        sprintf(send3, "{ \"k\":\"UnderV\",\"v\":\"%d\"},", Switch_Status.UnderVValue);
        strcat(send2, send3);
        sprintf(send3, "{ \"k\":\"OverC\",\"v\":\"%d\"},", Switch_Status.OverCValue);
        strcat(send2, send3);
        sprintf(send3, "{ \"k\":\"OverP\",\"v\":\"%d\"}", Switch_Status.OverPValue);
        strcat(send2, send3);
        strcat(send2, "]");
