...
void TFTLCD_ShowString(u16 x, u16 y, u8 *p, u16 fc, u16 bc, u8 sizey, u8 mode)
{
	while (*p != '\0')
	{
...
void TFTLCD_Image(u16 x, u16 y, u16 width, u16 height, const u8 *image)
{
	...
	for (i = 0; i < height; i++) // ???????
	{
		...
		for (j = 0; j < width; j++)							 // ???,????,??????
		{
			...
			TFTLCD_WR_DATA(colorData); // ??16?????
			cnt++;
		}
	}
}
void TFTLCD_ShowCN(u16 x, u16 y, u8 num, u16 fc, u16 bc, u8 size, u8 mode)
{
	u16 temp, t, t1;
	u16 y0 = y;
...
u8 tftlbkflag = 0;
u8 gettftblkstatus(void)
{
	return tftlbkflag;
}
void TFTBLK_DISABLE(void)
{
	tftlbkflag = 0;
	TFTLCD_BLK_RESET;
}
...
void TFTBLK_EN(void)
{
	tftlbkflag = 1;
	TFTLCD_BLK_SET;
	Vtimer_SetTimer(VTIM17, 60000, TFTBLK_DISABLE);
}
