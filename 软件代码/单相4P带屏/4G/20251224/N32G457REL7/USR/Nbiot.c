...
void NB_TXBufferInit(void)
{

	m_DataTxBuf = (u8 *)malloc(sizeof(u8));
	if (m_DataTxBuf != NULL)
	{
    printf("m_DataTxBuf InitOK \r\n");
	}else
	{
		printf("m_DataTxBuf InitERR \r\n");
	}
}

void NB_TXBufferFree(void)
{
		if (m_DataTxBuf != NULL)
		{
		  printf("m_DataTxBuf Free\r\n");
      free(m_DataTxBuf);
		}
}


void NB_LTEInit(void)
{

	LTE = (_LTE *)malloc(sizeof(_LTE));
	if (LTE != NULL)
	{
    printf("LTE InitOK \r\n");
	}else
