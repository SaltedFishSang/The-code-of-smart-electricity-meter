...
void Rtc_Init(void)
{
	...
	if (BKP_ReadBkpData(BKP_DAT1) != 0x0505)
	{ // 是否第一次配置？写过备份值就不执行RTC初始化 不掉电时间不变
...
void Get_RTC_Time(void)
{
  ...
  month1 = RtcTime.months;
  year1 = RTC_Date.Year +2000 ;
  if ((RTC_Date.Month == 1) || (RTC_Date.Month == 2))
  {
        month1 = RTC_Date.Month + 12;
        year1 = (u16)RTC_Date.Year + 2000 - 1;
  }
    RtcTime.weekday  = ((RTC_Date.Date + 2 * month1 + 3 * (month1 + 1) / 5 + year1 + year1 / 4 - year1 / 100 + year1 / 400) % 7) + 1;
