...
u8 SetPDP_Context(void)
{
    ...
    case 0:
        querynum++;
        NB_ClearRxBuf();
        LPUart_SendData("AT+QICSGP=1,1,\"CMNET\",\"\",\"\",1\r\n", 31); // 关闭回显
        NB_Start_TimeOutTimer(100);
        state = 1;
        return 0;
    case 1:
        ...
        if (NB_Check_TimeOutTimer()) // 超时时间到
        {
            ...
            if (p != NULL)
            {
                ...
                if (p1 != NULL)
                {
                    ...
                    Rtc_Timeset(h, m, s);
									  Rtc_Dateset(year-2000,month,day);
                    //  if ((h <= 59) && (m <= 59) && (s <= 59))
...
u8 MQTT_Creat(void)
{
    ...
    case 0:
        NB_ClearRxBuf();
        LPUart_SendData("AT+MQTTCFG=\"cached\",0,1\r\n", 25);
        NB_Start_TimeOutTimer(100);
        state = 1;
        return 0;
    case 1:
    ...
    case 2:
        NB_ClearRxBuf();
        LPUart_SendData("AT+MQTTCFG=\"keepalive\",0,120\r\n", 30);
        NB_Start_TimeOutTimer(100);
        state = 3;
        return 0;
    case 3:
    ...
    case 4:
        NB_ClearRxBuf();
        LPUart_SendData("AT+MQTTCONN=0,\"", 15);
        // sprintf(stringlen, "%d.%d.%d.%d\",", 8, 134, 155, 133);
        // sprintf(stringlen, "%d.%d.%d.%d\",", 39, 98, 162, 116);
        sprintf(stringlen, "%d.%d.%d.%d\",", 49, 234, 184, 198);
        LPUart_SendData((u8 *)stringlen, strlen(stringlen));
        sprintf(stringlen, "%d", 11080);
        LPUart_SendData((u8 *)stringlen, strlen(stringlen));
        LPUart_SendData(",\"", 2);
        LPUart_SendData(g_Equipment_number, 16);
        LPUart_SendData("\",", 2);
        LPUart_SendData("\"user\",\"0.123456\"\r\n", 19);
        NB_Start_TimeOutTimer(2000);
        state = 5;
        return 0;
    case 5:
    ...
    case 6:
        NB_ClearRxBuf();
        LPUart_SendData("AT+MQTTSUB=0,\"modbus_control/", 29);
    ...
    case 8:
        NB_ClearRxBuf();
        LPUart_SendData("AT+MQTTSUB=0,\"modbus_time/", 26);
        sprintf(stringlen, "%s", g_Equipment_number);
        LPUart_SendData((u8 *)stringlen, strlen(stringlen));
        LPUart_SendData("\",0\r\n", 5);
        NB_Start_TimeOutTimer(5000);
...
void LTE_ReceiveSendData(void)
{
    ...
    case 2:
        ...
        {
            NB_ClearRxBuf();
            LPUart_SendData("AT+MQTTREAD=0,1\r\n", 17);
            NB_Start_TimeOutTimer(100);
            state = 3;
        }
        break;
    ...
    case 6:
        ...
        break;
    case 7:
        LPUart_SendData("AT+MQTTPUB=0,\"", 14);
        LPUart_SendData(m_TopicTxBuf, m_TopicTxCnt);
        LPUart_SendData("\",", 2);
        LPUart_SendData("0,0,0,", 6);
        sprintf(stringlen, "%d", m_DataTxCnt);
        LPUart_SendData((u8 *)stringlen, strlen(stringlen));
        LPUart_SendData(",\"", 2);
        LPUart_SendData(m_DataTxBuf, m_DataTxCnt);
        LPUart_SendData("\"\r\n", 3);
