...
/*!
    \brief     EnableDog:打开�?件看门狗 WDI GPIO设置成推挽输出模�?
    \param[in]  none
    \param[out] none
    \retval     none
*/
...
/*!
    \brief      ModeInput_gpioInit(void) �?地远程模式选择开�?
    \param[in]  None
    \param[out] none
    \retval     none
*/
...
/*!
    \brief      EXTI0_1_IRQHandler(void) 按键�?�?函数
    \param[in]  None
    \param[out] none
    \retval     none
*/
void EXTI0_1_IRQHandler(void)
{
    ...
    {
        EXTI_ClrITPendBit(EXTI_LINE1);
        // if ((Modbus_ReadDI(3) == 0) && (Modbus_ReadDI(2) == 0)) // 手动模式�?  并且没有远程锁扣的情况下 才可以一�?分合�?
        if (Modbus_ReadDI(2) == 0) // 没有远程锁扣的情况下 才可以一�?分合�?
        {
            ...
            if (Modbus_ReadDI(1) == 1)
            {
                ...
                Modbus_WrieteInput(10, 9); // 再写入输入寄存器 最后一次分闸原�?  �?�?
                Flash_WirteModbusDI();
                ModbusSendAll();
            }
            else
...
void ModeInput_Check(void)
{
    ...
    //        //LedAlarmOff;
    //    }
    //    if (state != Modbus_ReadDI(3))
    //    {
    //        state = Modbus_ReadDI(3);
    //        ModbusSendAll();
    //    }
    if (Modbus_ReadDI(2) == 1)
    {
        LedAlarmOn;
    }
    if (Modbus_ReadDI(2) == 0)
    {
        LedAlarmOff;
    }
}
...
/*!
    \brief      Delay_ms(uint32_t count) �?秒延时函�?
    \param[in]  None
    \param[out] none
    \retval     none
*/
