...
void LPUartInit(void)
{
    ...
    LPUART_Init(LPUART1, &LPUART_InitStructure);
	    ...
	    /* Enable USARTy Receive and Transmit interrupts */
    LPUART_ConfigInt(LPUART1, USART_INT_RXDNE, ENABLE);
    /* Enable the USARTy and USARTz */
}  
...
/*!
    ...
    \retval     none
*/
void LPUart_SendData(u8 *data, u16 datalen)
{
    u16 i = 0;
    for (i = 0; i < datalen; i++)
    {
				LPUART_SendData(LPUART1,  (u16)data[i]);
       while (LPUART_GetFlagStatus(LPUART1, LPUART_FLAG_TXC) == RESET)
        ;
			  LPUART_ClrFlag(LPUART1, LPUART_FLAG_TXC);  
        // Delay_100uS(3);
...
void LPUART1_IRQHandler(void)
{
    ...
    {
        rec = LPUART_ReceiveData(LPUART1);
        In_Queue(MyQueue, rec);
    }
}
