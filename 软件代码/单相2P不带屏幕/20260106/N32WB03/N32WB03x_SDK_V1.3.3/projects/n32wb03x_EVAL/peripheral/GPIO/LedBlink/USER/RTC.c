...
void GetWeekday(void)
{
	u8 month1;
  u16 year1;
	month1 = RTC_DateStructure.Month;
  year1 = RTC_DateStructure.Year +2000 ;
  if ((RTC_DateStructure.Month == 1) || (RTC_DateStructure.Month == 2))
  {
        month1 = RTC_DateStructure.Month + 12;
        year1 = RTC_DateStructure.Year + 2000 - 1;
  }
    RTC_DateStructure.WeekDay  = ((RTC_DateStructure.Date + 2 * month1 + 3 * (month1 + 1) / 5 + year1 + year1 / 4 - year1 / 100 + year1 / 400) % 7) + 1;
}
/*!
    \brief      Rtc_Dateset(u8 year, u8 month, u8 date)
    \param[in]  None
    \param[out] none
    \retval     none
*/
void Rtc_Dateset(u8 year, u8 month, u8 date)
{
    RTC_DateDefault.Year = year;
    RTC_DateDefault.Month = month;
    RTC_DateDefault.Date = date;
    RTC_DateRegulate();
	  GetWeekday();
