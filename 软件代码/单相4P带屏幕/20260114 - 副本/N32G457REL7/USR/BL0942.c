...
void BL0942_PWRGpioInit(void)
{
    ...
    delay_ms(3000);
    BL0942_RegInit();
    Vtimer_SetTimer(VTIM4, 5000, ReadBL0942EleFlag);
}
/*!
...
void BL0942GetEleData(u8 *data)
{
    ...
    static u32 last_energry = 0; // 上一次的电能脉冲计数
    ...
    energytemp = BL0942_EleData_Reg.CF_CNT;//得到这次的脉冲值

    if (energytemp >= last_energry)//大于上次的脉冲值
    {
        mid_energy += (energytemp - last_energry);//把两次的差量值加上
    }
    else // 电能脉冲计数累积满出现溢出情况
    {
        mid_energy += (energytemp + (0xFFFFFF - last_energry));
    }
    last_energry = energytemp;
